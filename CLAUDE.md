...
### Each card gets its own git worktree, never the main checkout

The implementer never runs (or commits) directly in `--repo`. `branch_name_for_card` derives a deterministic branch name from the card (`agentloop/<slug-of-name>-<card-id-prefix>`), and `ensure_worktree` (in `claude_runner.py`) creates — or reuses, on a resumed card — an isolated `git worktree` for that branch under `--worktree-dir` (default: a `.agentloop-worktrees` folder next to `--repo`), branched off `--base-branch` (default: whatever branch `--repo` is on at startup). `implement()` runs `claude` with `cwd` set to that worktree, so commits land on the feature branch, and `--repo`'s working tree/HEAD are never touched while work is in progress. Because the branch name is derived purely from the card (not read back from comments), re-running the script after a restart reattaches to the same branch/worktree without any extra state.

On `review()` approval, `merge_branch` checks out `--base-branch` in `--repo` and merges the feature branch in (`--no-ff`), then `remove_worktree` deletes the worktree and the now-merged branch. If the merge fails (e.g. a conflict), the card is routed to **Needs Human** with the error in a comment instead of being marked Done, and the worktree is left in place for manual resolution. This is what keeps `--repo`'s main branch clean per-feature, per the Bun-post-inspired isolation this script is going for — `main` only ever receives fast, already-reviewed merges, never in-progress work.

### Why state lives in Trello comments, not a local file

After `implement()` runs, it records `before`/`after` git SHAs (plus the branch name) as a card comment: `[agentloop] before=<sha> after=<sha> branch=<branch>`. `review()` reads the SHAs back to compute the diff — this works from `--repo` even though the commits live on a branch checked out in a different worktree, since all worktrees of a repo share one object database. Review verdicts are likewise posted as `[review] APPROVE: ...` / `[review] REQUEST_CHANGES: ...` comments, and `count_request_changes` / `latest_request_changes_comment` parse the comment history to know how many rounds have happened and what feedback to feed back into the next `implement()` call. This makes the script fully stateless and restart-safe — killing and re-running it just re-derives state from the board (and the branch name is re-derived from the card, not parsed from comments).
